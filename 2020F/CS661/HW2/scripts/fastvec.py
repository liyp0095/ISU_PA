...
def main():
    ...
    model = FastText(size=100, window=5, min_count=1, sentences=common_texts, iter=20, \
                workers=4, sg=1)
    # sg ({0, 1}, optional) – Training algorithm: 1 for skip-gram; otherwise CBOW.
    # model = FastText(common_texts, size=100, window=9, min_count=1, workers=4, sg=1)
    with open("embedding/FastText_100_skip.txt", "w") as fp:
        line = []
